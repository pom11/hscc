...
/// Holds the user-configured connection settings: host + port in the shared
/// App-Group `UserDefaults` and the bearer token in the Keychain (never in
...
///
/// The host/port live in the APP GROUP suite (`group.com.hscc.ios`), NOT
/// `.standard`, so the widget and Live Activity extensions read the very same
/// values the operator enters here. The token is written to the app-group
/// Keychain access group so the extensions can see it too.
final class SettingsStore: ObservableObject {
    ...
    private static let hostKey = AppGroup.hostKey
