...
When it does tear down, it tells the watchdog to back off first, so your health-check daemon doesn't fight it and resurrect the layer mid-teardown. And be aware: this is brand-new and has never run a real teardown on this cluster — treat it as untested-at-scale until you've watched it cycle once.
