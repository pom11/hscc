"""hscc-roles themed-render helper — shared by the hscc.py CLI surface.

WHY
---
hscc-roles is converting its human-facing CLI output from bare ``print(...)``
/ ``json.dumps(...)`` to the themed Rich layer, matching the pool the
[1/5]..[5/5] Rich epic established for hscc_daemon. The single source of truth
for that palette is ``hscc_daemon.cli_theme`` (Hermes-default gold, dark +
light). This module is the hscc-roles-side handle on it.

hscc_daemon is a PEER plugin (+ a repo-root sibling dir), so it is on sys.path
in the ``hscc`` process and in the test workspace, but NOT necessarily in a
standalone hscc-roles install (plugins are NOT auto-importable siblings of each
other — only the script's own dir lands on sys.path). So the import is deferred
+ guarded (the same pattern flightdeck's commands/_theme.py uses): when
hscc_daemon is unavailable we fall back to a plain ``rich.console.Console`` so
hscc-roles still renders (just unthemed); only if ``rich`` itself is missing
does the module refuse to import.

EVERY hscc-roles Rich rendering goes through THIS module — no command re-derives
the guard or the palette.

HARD INVARIANT (inherited from the first epic): the machine ``--json`` /
``json.dumps`` paths stay BYTE-IDENTICAL (they still ``print(json.dumps)`` raw —
never routed through a Console). Non-TTY stdout degrades to PLAIN — Rich's
...
# standalone install, or any invocation where the repo root is not on sys.path —
# e.g. `cd hscc-roles && pytest`). It registers the SAME semantic names as the
# real palette so every card's markup / ``border_style`` resolves, but maps them
# to intentionally NEUTRAL styles (no colour), so the fallback stays genuinely
# unthemed while never crashing with MissingStyle. Without this, any card that
# renders ``border_style="error"`` or ``[error]`` / ``[dim]`` / ``[ok]`` /
...
def theme():
    ...
    Deferred + guarded so a standalone hscc-roles install (hscc_daemon not on
...
def make_console(_name: str | None = None, **kwargs) -> Console:
    ...
    file (piped / captured) Rich would otherwise default to 80, which makes any
    genuinely wider row (a long spec path, a multi-column role list) wrap
    mid-phrase or collapse a Rich Table's leading column to nothing. So when the
    resolved console is NOT a terminal and no explicit ``width=`` was given, it
    is recreated at width 200: content stays on one line / every column
    survives in a pipe, where preserving content beats wrapping. An explicit
    ``width=`` always wins.
    """
