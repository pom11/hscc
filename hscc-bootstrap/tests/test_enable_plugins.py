...
# ── telegram.enabled (TG-optional): the single fleet-wide switch ─────────────

def _fd_path(tmp_path):
    """Point _ensure_flightdeck_telegram at a tmp flightdeck config path."""
    return str(tmp_path / "flightdeck" / "config.yaml")


def _setup(tmp_path, monkeypatch, interview: str | None = "no", existing=None):
    """Redirect the flightdeck config to tmp + set the interview; write
    `existing` config text if provided."""
    import os as _os
    p = _fd_path(tmp_path)
    monkeypatch.setattr(enable_plugins, "_FLIGHTDECK_CONFIG", p)
    if interview is None:
        monkeypatch.delenv("HSCC_TELEGRAM", raising=False)
    else:
        monkeypatch.setenv("HSCC_TELEGRAM", interview)
...
def test_flightdeck_telegram_seeded_false_when_no(tmp_path, monkeypatch):
    """Fresh install, interview=no (the default) -> telegram.enabled: false."""
    p = _setup(tmp_path, monkeypatch, interview="no")
    changed = enable_plugins._ensure_flightdeck_telegram()
    assert changed == "telegram.enabled"
    cfg = yaml.safe_load(open(p))
    assert cfg["telegram"]["enabled"] is False


def test_flightdeck_telegram_seeded_true_when_yes(tmp_path, monkeypatch):
    """Fresh install, interview=yes -> telegram.enabled: true."""
    p = _setup(tmp_path, monkeypatch, interview="yes")
    changed = enable_plugins._ensure_flightdeck_telegram()
    assert changed == "telegram.enabled"
    cfg = yaml.safe_load(open(p))
...
def test_flightdeck_telegram_seeded_false_when_env_unset(tmp_path, monkeypatch):
    """No HSCC_TELEGRAM at all -> false (the documented no / fresh default)."""
    p = _setup(tmp_path, monkeypatch, interview=None)
    changed = enable_plugins._ensure_flightdeck_telegram()
    assert changed == "telegram.enabled"
    cfg = yaml.safe_load(open(p))
    assert cfg["telegram"]["enabled"] is False
...
def test_flightdeck_telegram_preserves_operator_true(tmp_path, monkeypatch):
    """An explicit operator enabled: true is NEVER overwritten, even with a
    no interview."""
    p = _setup(tmp_path, monkeypatch, interview="no",
               existing="telegram:\n  enabled: true\n  group_id: \"-100123\"\n")
    changed = enable_plugins._ensure_flightdeck_telegram()
...
def test_flightdeck_telegram_preserves_operator_false(tmp_path, monkeypatch):
    """An explicit operator enabled: false survives a yes interview."""
    p = _setup(tmp_path, monkeypatch, interview="yes",
               existing="telegram:\n  enabled: false\n")
    changed = enable_plugins._ensure_flightdeck_telegram()
    assert changed is None
    assert yaml.safe_load(open(p))["telegram"]["enabled"] is False


def test_flightdeck_telegram_idempotent_rerun(tmp_path, monkeypatch):
    """Re-running after a no-seed keeps enabled: false and does nothing else."""
    p = _setup(tmp_path, monkeypatch, interview="no")
    first = enable_plugins._ensure_flightdeck_telegram()
    assert first == "telegram.enabled"
    before = open(p).read()
    second = enable_plugins._ensure_flightdeck_telegram()
    assert second is None
    assert open(p).read() == before


def test_flightdeck_telegram_malformed_config_untouched(tmp_path, monkeypatch):
    """A malformed existing flightdeck config is left alone (never clobbered)."""
    p = _setup(tmp_path, monkeypatch, interview="no",
               existing="telegram: [unclosed\n  this: not: valid\n")
    changed = enable_plugins._ensure_flightdeck_telegram()
    assert changed is None
    assert "not: valid" in open(p).read()